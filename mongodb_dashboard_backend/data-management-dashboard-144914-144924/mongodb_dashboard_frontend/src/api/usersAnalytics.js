...
export async function getActiveUsersTrend(params = {}) {
  ...
  const primaryUrl = `${base}/users/active-trend`;
  try {
    const res = await axios.get(primaryUrl, { params });
    return res.data;
  } catch (err) {
    if (err?.response?.status === 404) {
      // fallback to canonical analytics path if users path is not mounted
      const altUrl = `${base}/analytics/users/activity-trend`;
      const res2 = await axios.get(altUrl, { params });
      return res2.data;
    }
    throw err;
  }
...
export async function getTenantUsersSummary(params = {}) {
  ...
  try {
    ...
    if (err?.response?.status === 404) {
      // Fallback to canonical analytics alias
      const altUrl = `${base}/analytics/users/tenant-summary`;
      const res2 = await axios.get(altUrl, { params });
      const data2 = res2?.data ?? res2;
      if (data2 && Array.isArray(data2.items)) {
        return { items: data2.items, total: data2.total ?? data2.items.length };
      }
      if (Array.isArray(data2)) {
        return { items: data2, total: data2.length };
      }
      if (data2 && typeof data2 === "object") {
        const items = Array.isArray(data2.data) ? data2.data : Array.isArray(data2.items) ? data2.items : [];
        return { items, total: data2.total ?? items.length ?? 0 };
      }
      return { items: [], total: 0 };
    }
