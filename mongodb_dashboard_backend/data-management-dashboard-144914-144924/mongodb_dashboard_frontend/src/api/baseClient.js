...
/**
 * PUBLIC_INTERFACE
 * Fetch distinct values for a field from session tracking.
 * field: "User_name" | "tenant_id"
 * Supports optional date range (start/end) and filter scoping consistent with listSessions.
 * Returns a simple array of strings.
 */
export async function listSessionDistinct(field, params = {}) {
  const res = await httpGet("/api/session-tracking/distinct", {
    params: { field, ...(params || {}) },
  });
