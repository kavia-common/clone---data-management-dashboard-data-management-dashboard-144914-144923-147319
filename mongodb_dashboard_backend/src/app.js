...
safeUse('/api/costs', require('./routes/costs.byAgent.routes'));
 // LLM costs routes are mounted through baseRouter with verifyAuth + requireTenant to enforce tenant scoping.
 // Avoid double mounting here to prevent bypass of middleware or duplicate handlers.
 // If public docs or hierarchy routes are needed separately, mount them via baseRouter or ensure same middleware.
