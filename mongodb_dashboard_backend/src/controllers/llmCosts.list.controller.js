...
async function list(req, res) {
  // Top-level guard for any unexpected sync errors, ensuring JSON response instead of crashing
  try {
    ...
    // Resolve tenant or return 400 if required
    const bypass = !!(req.tenantScopeDisabled || req.allTenants || req.costsAllTenantsBypass);
    let resolvedTenant = null;
    if (!bypass) {
      resolvedTenant =
        (req.tenantId && String(req.tenantId)) ||
        (req.organizationId && String(req.organizationId)) ||
        (req.auth?.tenantId ? String(req.auth.tenantId) : null) ||
        null;
      // As last resort, accept aliases when middleware didn't set
      if (!resolvedTenant) {
        const hdr =
    ...
    const baseProject = {
      ...
      agent_name: 1,
      'metadata.agent': 1,
      // Access 'Agent Name' safely only if metadata is object via $getField in aggregation; omit here in find() projection
      total_cost: 1,
      cost_usd: 1,
      'cost.amount': 1,
      'cost.currency': 1,
    ...
    try {
      ...
      pipeline.push({
        $project: {
          ...
          // Normalize cost selection in aggregation context only
          _total_cost_norm: {
            $ifNull: ['$total_cost', { $ifNull: ['$cost_usd', 0] }],
          },
        },
      });
    ...
    // Truly unexpected synchronous failure
