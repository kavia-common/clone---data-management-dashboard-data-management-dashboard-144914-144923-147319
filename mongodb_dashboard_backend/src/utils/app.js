...
function tryRequireRoute(modulePath, options = {}) {
  ...
  try {
    ...
    if (!router) throw new Error('Module returned falsy router');
    return router;
  } catch (err) {
    // eslint-disable-next-line no-console
    console.error(`[routes:safe] Failed to load route module ${label}. Continuing with stub.`, err?.message || err);
