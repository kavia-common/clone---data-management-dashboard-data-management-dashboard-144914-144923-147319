...
- PORT: Express server port. Defaults to 7001 if not set.
- HOST: Bind address. Defaults to 0.0.0.0.

The server exposes OpenAPI JSON at:
- http://localhost:${PORT}/openapi.json (e.g., http://localhost:7001/openapi.json)
...
1. Start the backend and check logs:
   - Look for "[startup] Express listening on http://0.0.0.0:7001 ..." and DB connection logs.
2. Ensure your MongoDB database actually has documents in the 'llm_costs' collection.
3. Call:
   - GET http://localhost:7001/api/llm-costs
   - GET http://localhost:7001/api/llm-costs?page=1&limit=10
   Expect non-empty results when data exists.
4. Verify users trend:
   - GET http://localhost:7001/api/users/active-trend
