...
// PUBLIC_INTERFACE
// oceanTheme
// Ocean Professional theme helpers for categorical palettes, typography, and surfaces.
 ...
 * @param count Number of distinct categories needed.
 * @returns string[] hex colors
 */
export function getCategoricalPalette(count: number): string[] {
  const { colors } = getOceanTheme();
  // Seed palette with brand colors, then expand with tints/shades
  const seed = [
    colors.primary,
    colors.secondary,
    "#10B981",
  ...
  // Generate additional variants by adjusting lightness via HSL conversion
  ...
  while (out.length < count) {
    ...
    const delta = ((Math.floor(out.length / seed.length) + 1) * 0.08) % 0.36; // stagger lightness
...
export function getCategoryColorMap(categories: string[]): Record<string, string> {
  const palette = getCategoricalPalette(categories.length || 1);
